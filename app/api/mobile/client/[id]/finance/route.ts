...
export async function GET(request: Request, context: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const [clientResult, financeResult, paymentsResult, additionsResult] = await Promise.all([
    ...
    admin.from("project_payments").select("id, amount, payment_date, note").eq("client_id", clientId).order("payment_date", { ascending: false }).order("created_at", { ascending: false }),
    admin.from("project_additions").select("id, title, amount, addition_date, note").eq("client_id", clientId).order("addition_date", { ascending: false }).order("created_at", { ascending: false }),
  ]);
  if (clientResult.error) return NextResponse.json({ error: "المشروع غير موجود" }, { status: 404 });
  return NextResponse.json({ project_name: clientResult.data?.project_name ?? "مشروعك", finance: financeResult.data ?? null, payments: paymentsResult.data ?? [], additions: additionsResult.data ?? [] }, { headers: { "Cache-Control": "no-store" } });
}
