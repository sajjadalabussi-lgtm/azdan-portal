...
export async function GET(request: Request, context: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const { admin } = session;
  const { data, error } = await admin.from("project_files").select("id, title, description, category, storage_path, file_name, created_at").eq("client_id", clientId).eq("is_visible_to_client", true).order("created_at", { ascending: false });
